<?php
/*
Plugin Name: WP-PostViews for Vmeng
Plugin URI: https://blog.iknet.top/post/vmeng-final.html
Description: 文章计数插件WP-PostViews Vmeng兼容版
Version: 1.75
Author: 秋のかえで & 'GaMerZ' Chan
Author URI: https://iknet.top
*/

/*
原插件地址: https://lesterchan.net/portfolio/programming/php/
原作者: 'GaMerZ' Chan
原作者地址: lesterchan.net
*/
...
### Function: Calculate Post Views With WP_CACHE Enabled(兼容Vmeng)
add_action('wp_footer', 'wp_postview_cache_count_enqueue');
...
*/
	...
	if ( !wp_is_post_revision( $post ) && ( is_single() || is_page() ) ) {
		...
		if ( $should_count ) {
			...
			$script = json_encode( array( 'admin_ajax_url' => admin_url( 'admin-ajax.php' ), 'post_id' => intval( $post->ID ) ) );
			echo '<script type=\'text/javascript\' data-no-instant>';
			echo '/* <![CDATA[ */';
			echo "var viewsCacheL10n = ".$script.';';
			echo '/* ]]> */';
			echo '</script>';
			echo ' <script type="text/javascript" src="'.plugins_url( 'postviews-cache.js', __FILE__ ).'" data-no-instant></script>';
		}
	}
}
